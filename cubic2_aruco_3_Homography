...
while True:
    ...
    if real_world_red:
        # 当前红球位置：红色圆圈
        cv2.circle(display, (int(real_world_red[1]), int(real_world_red[0])), 8, (0, 0, 255), -1)
        #cv2.putText(display, "Current", (real_world_red[1] + 10, real_world_red[0] - 10),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)

        # 查找 real_world_path 中最接近 real_world_red 的点（注意 real_world_red 仍是图像坐标）
        if real_world_path:
            distances = [calculate_distance(real_world_red, (pt[1], pt[0])) for pt in real_world_path]
            ...
            if target_idx < len(real_world_path):
                ...
                # 在图像上用投影方式标记目标点位置（可选：如你想画出来）
                target_px = (int(target_point[0]), int(target_point[1]))  # (x, y)
                ...
                # 误差（仍基于像素图像）
                euclidean_error = calculate_distance(real_world_red, (int(nearest_path_point[1]), int(nearest_path_point[0])))
                ...
                #send_two_points_16bit(real_world_red[1], real_world_red[0],int(target_point[0]), int(target_point[1]))

                x__1 = real_world_red[1]
                x__2 = int(target_point[0])
                y__1 = real_world_red[0]
                y__2 = int(target_point[1])
