...
from scipy.interpolate import splprep, splev #cubic b spline
...
def generate_path_overlay(image):
    ...
    for idx, point in enumerate(non_zero_points):
        if idx not in used_indices:
            ...
            used_indices.update(kdtree.query_ball_point(point, r=30))
    ...
    # ===== B样条平滑路径：使用 cubic B-spline 插值 =====
    if len(ordered_path) >= 4:  # 至少要4个点才能拟合三次样条
        path_array = np.array(ordered_path)
        y_points, x_points = path_array[:, 0], path_array[:, 1]

        # 使用 B-spline 拟合路径（参数 s 控制平滑度）
        tck, u = splprep([x_points, y_points], s=5, k=3)

        # 生成细化点（越多越平滑）
        u_fine = np.linspace(0, 1, len(ordered_path) * 5)  # 每段插5个点
