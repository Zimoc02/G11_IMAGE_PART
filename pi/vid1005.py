...
tracking_roi = None
percentage_error_x = 0
percentage_error_y = 0
...
while True:
        ...
        if w > 0 and h > 0:
            ...
            for contour in contours:
                if cv2.contourArea(contour) > 200:
                    ...
                    AUX_RADIUS = 3
                    AUX_POINTS_PER_CIRCLE = 30  # 从12增加到30点，形成更完整的圆周覆盖
    ...
    if farthest_point and position_history:
        ...
        percentage_error_x = dx/current_pos[0]*100
        percentage_error_y = dy/current_pos[1]*100
        print(f"errorx:{percentage_error_x}" f"errory:{percentage_error_y}")
    ...
    for idx, offset in enumerate([2, 4, 6, 8, 10]):
        if len(position_history) >= offset:
            ...
            cv2.putText(frame, f"T-{offset}: {pos}:{percentage_error_x}:{percentage_error_y}",  (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
